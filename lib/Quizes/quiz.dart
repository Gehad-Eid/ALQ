...
class quiz_page extends StatelessWidget {
    ...
    return Scaffold(
      ...
      actions: [
        Countdown(
          controller: _count,
      seconds: 10,
      build: (BuildContext context, double time) => Text(time.toString(), style: TextStyle(fontSize: 23),),
      ...
      onFinished: () {
       if(total >9){
        ...
        _page.nextPage(duration: Duration(microseconds: 1), curve: Curves.bounceIn);
       total +=1;
       print(total);
       _count.restart();
       }
  ...
  wrongAnswers: ["${data['Q$index']['answers'][1]}","${data['Q$index']['answers'][2]}","${data['Q$index']['answers'][3]}",],
  onRightAnswer: (){
       score += 1;
       total += 1;
       print(score);
